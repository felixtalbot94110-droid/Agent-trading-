...
# ================================================================
#  CONFIG - Tout est deja configure !
# ================================================================
...
WATCHLIST_FULL = [
    ...
    {"symbol": "USO",     "name": "Petrole (ETF)",  "type": "Énergie", "currency": "$"},
...
def get_active_watchlist():
    ...
    if is_weekend():
        log.info("[DATE] Week-end detecte -> scan crypto uniquement (ZEC, SOL)")
...
def init_db():
    ...
    conn.commit()
    conn.close()
    log.info("[OK] Base de donnees OK")
...
def compute_score(rsi, macd, macd_sig, ema20, ema50):
    ...
    if macd is not None and macd_sig is not None:
        ...
        sig = "Haussier [BUY]" if macd > macd_sig else "Baissier [SELL]"
        pts += p
        details["macd"] = {"value": round(macd, 3), "signal": sig}

    if ema20 is not None and ema50 is not None:
        p = 30 if ema20 > ema50 else 5
        sig = "Tendance haussiere [BUY]" if ema20 > ema50 else "Tendance baissiere [SELL]"
        pts += p
...
def claude_analyze(asset, price, chg, score, details):
    ...
    prompt = f"""Tu es un expert en swing trading. Analyse cet actif et genere un signal precis.
...
def send_signal_with_buttons(asset, price, chg, score, signal_data):
    ...
    emoji  = {"BUY": "[BUY]", "SELL": "[SELL]", "WAIT": "[WAIT]"}.get(sig, "[WAIT]")
    action = {"BUY": "ACHAT", "SELL": "VENTE", "WAIT": "ATTENDRE"}.get(sig, "SIGNAL")
    c_emo  = {"fort": "[FORT]", "moyen": "[MOYEN]", "faible": "[ATTENTION]"}.get(
               signal_data.get("confiance", "moyen"), "[MOYEN]")
    ...
    text = (
        ...
        f"----------------------\n"
        f"[CAPITAL] Prix : <b>{cur}{price:.2f}</b> ({chg:+.2f}%)\n"
        f"[SCORE] Score : <b>{score}/100</b>\n\n"
        f"[PLAN] <b>PLAN DE TRADE</b>\n"
    ...
    keyboard = {"inline_keyboard": [[
        ...
        {"text": "[WAIT] Je surveille", "callback_data": f"surveille|{asset['symbol']}"},
    ]]}
    ...
    if resp and resp.get("ok"):
        ...
        log.info(f"[OK] Signal envoye (msg_id={msg_id})")
        return msg_id
    return None
...
def edit_after_decision(message_id, original, decision):
    emoji = {"pris": "[OK]", "passe": "[NON]", "surveille": "[WAIT]"}.get(decision, "-")
    label = {"pris": "PRIS", "passe": "PASSÉ", "surveille": "SURVEILLÉ"}.get(decision, decision)
    new_text = f"{original}\n\n{emoji} <b>Decision : {label}</b> - {datetime.now().strftime('%H:%M')}"
...
def send_startup():
    ...
    mode = "[WEEKEND] Mode week-end - Crypto uniquement" if is_weekend() else "[LONG] Mode semaine - Tous les actifs"
    send_simple(
        f"[BOT] <b>Trading IA by fefe - Demarre !</b>\n"
        f"----------------------\n"
        f"[LIST] Watchlist complete : {symbols}\n"
        f"[TIME] Scan toutes les {SCAN_INTERVAL_MIN} min\n"
        f"[TARGET] Seuil d'alerte : {SCORE_THRESHOLD}/100\n"
        f"[CAPITAL] Capital : {CAPITAL_TOTAL}€ · Risque : {RISK_PER_TRADE_PCT}%\n"
        f"[DATE] {mode}\n"
        f"[HEURE] {datetime.now().strftime('%d/%m/%Y %H:%M')}\n"
        f"----------------------\n"
        f"En attente de signaux... [SCAN]\n\n"
...
def send_daily_report():
    ...
    for t in trades:
        ...
        de = {"pris":"[OK]","passe":"[NON]","surveille":"[WAIT]","pending":"❓"}.get(dec,"❓")
        se = {"BUY":"[BUY]","SELL":"[SELL]","WAIT":"[WAIT]"}.get(sig,"-")
        recent += f"  {de} {se} {sym} | Entry:{entry} | Score:{score}\n"
    send_simple(
        f"[SCORE] <b>Rapport - {datetime.now().strftime('%d/%m/%Y')}</b>\n"
        f"----------------------\n"
        f"Signaux envoyes : <b>{stats['total']}</b>\n"
        f"[OK] Pris       : <b>{stats['pris']}</b>\n"
        f"[NON] Passes     : <b>{stats['passe']}</b>\n"
        f"[WAIT] Surveilles : <b>{stats['surveille']}</b>\n"
        f"❓ En attente : <b>{stats['pending']}</b>\n\n"
        f"[LIST] <b>Derniers signaux :</b>\n"
...
def telegram_polling():
    offset = 0
    log.info("[POLL] Polling Telegram demarre...")
        ...
        try:
            ...
            for upd in updates:
                ...
                if "callback_query" in upd:
                    ...
                    labels = {"pris": "[OK] Trade pris !",
                              "passe": "[NON] Ignore",
                              "surveille": "[WAIT] En surveillance"}
                    ...
                    log.info(f"Decision : {decision} sur {symbol}")

                    if decision == "pris":
                        send_simple(
                            f"[OK] <b>Trade {symbol} enregistre !</b>\n\n"
                            f"👉 <b>À faire sur XTB maintenant :</b>\n"
                            f"  1. Ouvre l'app XTB\n"
                            f"  2. Cherche <b>{symbol}</b>\n"
                            f"  3. Place l'ordre avec le stop et le target\n\n"
                            f"Bonne chance ! [FORT][LONG]"
                        )
                    elif decision == "surveille":
                        send_simple(
                            f"[WAIT] <b>{symbol} en surveillance</b>\n"
                            f"Je continue a monitorer cet actif.\n"
                            f"Tu seras alerte si le setup evolue."
                        )
                ...
                # ── Commande texte ─────────────────────────────
                    ...
                    if text == "/stats":
                        ...
                        mode = "[WEEKEND] Week-end - Crypto only" if is_weekend() else "[LONG] Semaine - Tous les actifs"
                        syms = "\n".join(
                            [f"  • {a['symbol']} - {a['name']}" for a in active])
                        send_simple(f"[LIST] <b>Watchlist active</b> ({mode})\n{syms}")

                    elif text == "/status":
                        stats = get_trade_stats()
                        send_simple(
                            f"[BOT] <b>Agent actif</b>\n"
                        ...
                        send_simple(
                            f"[BOT] <b>Trading IA by fefe - Commandes</b>\n"
                            f"----------------------\n"
                            ...
                            f"avec des boutons pour repondre 👇"
                        )
...
def daily_report_scheduler():
    while True:
        ...
        if now.hour == 18 and now.minute == 0:
            log.info("[SCORE] Rapport journalier...")
            send_daily_report()
            time.sleep(61)
        time.sleep(30)
...
def scan_asset(asset):
    ...
    if not quote or "close" not in quote:
        log.warning(f"{symbol} : pas de donnees (marche ferme ?)")
        return
    ...
    if score >= SCORE_THRESHOLD and can_alert(symbol):
        log.info(f"[TARGET] {symbol} score {score} ≥ {SCORE_THRESHOLD} -> Claude analyse...")
        ...
        if signal and signal.get("signal") in ("BUY", "SELL"):
            ...
            if msg_id:
                ...
                log.info(f"[OK] {signal['signal']} envoye pour {symbol}")
        else:
...
def run_scan_cycle():
    ...
    log.info(f"{'='*50}")
    active = get_active_watchlist()
    log.info(f"[LIST] {len(active)} actifs a scanner {'(mode week-end [WEEKEND])' if is_weekend() else '(mode semaine [LONG])'}")
    for asset in active:
        scan_asset(asset)
        time.sleep(1.5)
...
def main():
    log.info("[START] Trading IA by fefe - Demarrage")
    log.info(f"[LIST] {len(WATCHLIST_FULL)} actifs | Seuil: {SCORE_THRESHOLD} | Scan: {SCAN_INTERVAL_MIN}min")
    ...
    while True:
        try:
            if is_trading_hours():
                ...
                log.info(f"[PAUSE] Hors heures ({MARKET_OPEN_HOUR}h-{MARKET_CLOSE_HOUR}h) - pause")

            log.info(f"[PAUSE] Prochain scan dans {SCAN_INTERVAL_MIN} min...")
            time.sleep(SCAN_INTERVAL_MIN * 60)

        except KeyboardInterrupt:
            log.info("[STOP] Agent arrete")
            send_simple("[STOP] <b>Trading IA by fefe - Arrete</b>")
            break
        except Exception as e:
            log.error(f"Erreur : {e}")
            send_simple(f"[ATTENTION] <b>Erreur agent</b> : {e}")
            time.sleep(60)
